...
func CreateFlogoTrigger(configDefinitions map[string]types.Config, trigger types.Trigger, namedHandlerMap map[string]types.EventHandler,
	...
	if util.CheckTriggerOptimization(mashTriggerSettingsUsable) {
		fmt.Printf("Trigger specifies %v property setting true", util.Gateway_Trigger_Optimize_Property)
		...
		for _, name := range triggerNames {
			...
			if reflect.DeepEqual(createdTrigger.Settings, triggerSettings) {
				...
				fmt.Printf("Found a trigger having same settings %v %v ", name, triggerSettings)
				flogoTrigger = *createdTrigger
				isNew = false
				break
			} else {
				fmt.Printf("Current trigger %v did not match settings of trigger %v %v", flogoTrigger.Name, name, triggerSettings)
			}
		}
	}
	...
	for _, dispatch := range dispatches {
		...
		if dispatch.If != "" {
			...
			if util.IsValidTriggerHandlerSetting(triggerMD, util.Flogo_Trigger_Handler_Setting_Condition) {
				...
				fmt.Printf("The trigger [%v] does not support [%v] handler setting. skippng the condition logic.", trigger.Type, util.Flogo_Trigger_Handler_Setting_Condition)
			}
		}
	...
	if isNew {
		fmt.Printf("Adding a new trigger with settings %v %v ", flogoTrigger.Name, triggerSettings)
