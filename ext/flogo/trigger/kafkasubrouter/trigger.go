...
import (
	...
	"github.com/mashling/commons/lib/conditions"
	"github.com/mashling/commons/lib/util"
)

// log is the default package logger
var flogoLogger = logger.GetLogger("trigger-tibco-kafkasubrouter")

// map of handlers and condition operators. cache the operation so that the message handling is quick.
var handlerConditionMap = make(map[string]*condition.Condition)
...
func onMessage(t *KafkaSubTrigger, msg *sarama.ConsumerMessage) {
	...
	for _, handler := range t.config.Handlers {
		if handler.Settings[util.Flogo_Trigger_Handler_Setting_Condition] != nil {
			...
			*/
			//check if the message content is JSON first. mashling only supports JSON payloads for condition/content evaluation right now
			data := string(msg.Value)
			isJson := condition.IsJSON(data)
			if !isJson {
